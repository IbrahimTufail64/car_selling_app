...
const SurfaceMarks = () => {
    ...
    const handleSubmit = async (event:any) => { 
        ...
        try {
          ...
          const response1 = await axios.post(`${url}/pwa/panel_damage_form`,  
            formData, {
                headers: {
                    Authorization: `Bearer ${token}`
                  }
          });
          const response2 = await axios.post(`${url}/pwa/panel_damage_data`,  
            {
                damage: newArr,
                car_no: car 
            }, {
    ...
    useEffect(()=>{
        localStorage.setItem('prevRoute','./panel_damage');
        ...
        retrieve('panel_damage',setImages);
  ...
  return (
    <div className={`${isVendor ? 'bg-primaryDark text-white' : 'bg-secondary'} w-full min-h-[100vh] pb-[90px]`}>
        ...
        <div className={`w-full flex justify-center ${isVendor && 'text-primaryDark'}`}>
            <div className='w-[90vw] bg-[#D1D9FF] overflow-hidden mt-7 pl-3 pt-3 flex justify-between rounded-lg'>
                <div className='space-y-5'>
                    <div className='font-[300] text-sm'>Perfect your car’s panel damage photos with our expert guide.</div>
                    <Link  href='./advice_panel_damage'  className='font-[400] text-sm mt-5'>Smart advice &gt;</Link>
                </div>
        ...
        <div className='space-y-3 pt-7'>
            {images.length === 0 && <PhotoFrameDynamic image_name='panel_damage' Car_no={car_no} DynamicImageNo={1} Content='Panel damage' isUploaded={false} photo={ ExampleImage}  return_link ='panel_damage' updateState={updateState}/>}
        <div className="embla overflow-hidden mx-2">
        <div className="embla__viewport" ref={emblaRef}>
          <div className="embla__container flex space-x-5">
            {images.map((e,i)=>{
                return <div className="embla__slide "><PhotoFrameDynamic image_name='panel_damage' Car_no={car_no} DynamicImageNo={Number(e.dynamic_image_number)} Content='Panel damage' isUploaded={e !== null} photo={ e ? e.data : ExampleImage}  return_link ='panel_damage' updateState={updateState}/></div>;
            })}
            {images.length>=1 && 
                <PhotoFrameDynamic image_name='panel_damage' Car_no={car_no} DynamicImageNo={images.length+1} Content='Panel damage' isUploaded={false} photo={ ExampleImage}  return_link ='panel_damage' updateState={updateState}/>
            }
          </div>
        </div>
        ...
        <Link href={`./camera_filter_dynamic/${'panel_damage'}-${images.length+1}-${'panel_damage'}`} className='py-2 px-5 text-[18px] my-5'>
             Add another photo
        </Link>
        </div> */}
        </div>
