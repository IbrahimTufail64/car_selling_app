...
const SurfaceMarks = () => {
    ...
    useEffect(()=>{
        localStorage.setItem('prevRoute','./glass_health');
        ...
        retrieve('glass_health',setImages);
  ...
  return (
    <div className={`${isVendor ? 'bg-primaryDark text-white' : 'bg-secondary'} w-full min-h-[100vh] pb-[50px]`}>
        ...
        <div className={`w-full flex justify-center ${isVendor && 'text-primaryDark'}`}>
            <div className='w-[90vw] bg-[#D1D9FF] overflow-hidden mt-7 pl-3 pt-3 pb-2 flex justify-between rounded-lg'>
                <div className='space-y-5'>
                    <div className='font-[300] text-sm'>Not sure what windscreen damage looks like? Check out our guide!</div>
                    <Link  href='./advice_glass_health'  className='font-[400] text-sm mt-5'>Smart advice &gt;</Link>
                </div>
        ...
        <div className='space-y-3 pt-7'>
            {images.length === 0 && <PhotoFrameDynamic image_name='glass_health' Car_no={car_no} DynamicImageNo={1} Content='Glass health' isUploaded={false} photo={ ExampleImage}  return_link ='glass_health' updateState={updateState}/>}
        <div className="embla overflow-hidden mx-2">
        <div className="embla__viewport" ref={emblaRef}>
          <div className="embla__container flex space-x-5">
            {images.map((e,i)=>{
                return <div className="embla__slide "><PhotoFrameDynamic image_name='glass_health' Car_no={car_no} DynamicImageNo={Number(e.dynamic_image_number)} Content='Glass health' isUploaded={e !== null} photo={ e ? e.data : ExampleImage}  return_link ='glass_health' updateState={updateState}/></div>;
            })}
            {images.length>=1 && 
                <PhotoFrameDynamic image_name='glass_health' Car_no={car_no} DynamicImageNo={images.length+1} Content='Glass health' isUploaded={false} photo={ ExampleImage}  return_link ='glass_health' updateState={updateState}/>
            }
          </div>
        </div>
        ...
        <Link href={`./camera_filter_dynamic/${'glass_health'}-${images.length+1}-${'glass_health'}`} className='py-2 px-5 text-[18px] my-5'>
             Add another photo
        </Link>
        </div> */}
        </div>
