  ...
  return (
    <div className={`${isVendor ? 'bg-primaryDark text-white' : 'bg-secondary'} w-full min-h-[100vh] pb-[50px]`}>
        ...
        <div className='space-y-3 pt-7'>
            {images.length === 0 && <PhotoFrameDynamic image_name='dashboard_lights' Car_no={car_no} DynamicImageNo={1} Content='Dashboard and lights' isUploaded={false} photo={ ExampleImage}  return_link ='dashboard_lights' updateState={updateState}/>}
        <div className="embla overflow-hidden mx-2">
        <div className="embla__viewport" ref={emblaRef}>
          <div className="embla__container flex space-x-5">
            {images.map((e,i)=>{
                return <div className="embla__slide "><PhotoFrameDynamic image_name='dashboard_lights' Car_no={car_no} DynamicImageNo={Number(e.dynamic_image_number)} Content='Dashboard and lights' isUploaded={e !== null} photo={ e ? e.data : ExampleImage}  return_link ='dashboard_lights' updateState={updateState}/></div>;
            })}
            {images.length>=1 && images.length <3 && 
                <PhotoFrameDynamic image_name='dashboard_lights' Car_no={car_no} DynamicImageNo={images.length+1} Content='Dashboard and lights' isUploaded={false} photo={ ExampleImage}  return_link ='dashboard_lights' updateState={updateState}/>
            }
          </div>
        </div>
