...
const PhotoFrameReturn = ({Content, isUploaded, photo, link,updateState,index,return_link}:{Content:string, isUploaded:Boolean, photo: any, link:string,index:number, updateState: (index:number, value: boolean) => void,return_link:string}) => {
  ...
  return (
    <div className={`${isVendor ? 'bg-primaryDark' : 'bg-secondary  '} flex justify-center  w-full text-[#0C0D45]`}>
            ...
            <Link href={`${!isUploaded ? `./camera_filter/${link}-${return_link}` : '#'}`} onClick={()=>{isUploaded && handleDelete()}} className={`py-4 px-5 text-[18px] flex justify-between  ${isVendor ? 'bg-[#D1D9FF] ' : ''}`}>
                ...
                <div className='pt-1'>
                    {isUploaded ? <img src={Delete.src} onClick={handleDelete}/>: <Link href={`./camera_filter/${link}-${return_link}`}><img src={Camera.src} /></Link>}
                </div>
            </Link>
        </div>
    </div>
...
export default PhotoFrameReturn